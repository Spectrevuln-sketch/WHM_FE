...
import { getInventory } from "../../../../(master)/master-inventory/@usecase/handler";
import { useAppDispatch, useAppSelector } from "@/store/store";
import {  setSelectedInput } from "@/store/msr/slice";
import { ProductInterface } from "@/components/modals/AddMsrProductModal";
import { disableSubmit } from "@/helpers/validationForm";
import { getCurrentMsr } from "./handler";
...
export const useUpdateMsr = ({msrId }:IProps): IReturn =>{
  ...
  console.log('data msr current >> ', payload)
  ...
  const getMsr = async (id:string) =>{
    const currentMsr = await getCurrentMsr(id)
    console.log('current MSR ????\n', currentMsr)
    setPayload({
      ...currentMsr,
      msr_number: currentMsr.msr_number,
      depts: currentMsr.depts.id,
      // deliveryDate: moment(currentMsr.delivered_at).utc().format('MM/DD/YYYY').toString(),
      urgency: convertToCapitalcase(currentMsr.urgentcy),
    })
    // setSelectedProducts(currentMsr.list_of_items)
}
